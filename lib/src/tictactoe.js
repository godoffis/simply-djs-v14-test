        ...
        return new Promise((resolve) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const acceptEmbed = new discord_js_1.EmbedBuilder()
                    ...
                    ? (_d = options.embed) === null || _d === void 0 ? void 0 : {
                        text: _d.footer
                    }
                    : {
                ...
                collector.on('collect', (button) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (button.customId === 'deny-ttt') {
                        ...
                        const epm = new discord_js_1.EmbedBuilder()
                            .setTitle(`${message.member.user.username} vs ${opponent.username}`)
                            ...
                            ? (_u = options.embed) === null || _u === void 0 ? void 0 : {
                                text: _u.footer
                            }
                            : {
                                text: '©️ Simply Develop. npm i simply-djs',
                            ...
                            return __awaiter(this, void 0, void 0, function* () {
                                ...
                                if (won['O-Player'] != false) {
                                    const wonner = yield client.users
                                        ...
                                        .catch(console.error);
                                    resolve(wonner);
                                    if (!options.result || options.result === 'Button')
                                        return m
                                            .edit({
                                ...
                                if (won['X-Player'] != false) {
                                    ...
                                    if (!options.result || options.result === 'Button')
                                        ...
                                        return m
                                            ...
                                            content: `<@${players[Plrs.user === 0 ? 1 : 0]}> (${client.emojis.cache.get(o_emoji) || '❌'}) won`,
                                            embeds: [
                                                discord_js_1.EmbedBuilder.from(epm).setDescription(`<@!${players[Plrs.user === 0 ? 1 : 0]}> (${client.emojis.cache.get(x_emoji) || '❌'}) won.. That was a nice game.\n` +
                                                    `\`\`\`\n${Plrs.a1.emoji} | ${Plrs.a2.emoji} | ${Plrs.a3.emoji}\n${Plrs.b1.emoji} | ${Plrs.b2.emoji} | ${Plrs.b3.emoji}\n${Plrs.c1.emoji} | ${Plrs.c2.emoji} | ${Plrs.c3.emoji}\n\`\`\``
                                                        .replaceAll(dashmoji, '➖')
                                                        .replaceAll(o_emoji, '⭕')
                                                        .replaceAll(x_emoji, '❌'))
                                ...
                                const tttcollector = m.channel.createMessageComponentCollector({
                                    componentType: discord_js_1.ComponentType.Button,
                                    max: 1,
                                    time: 30000
                                });
                                tttcollector.on('collect', (b) => __awaiter(this, void 0, void 0, function* () {
                                    if (b.user.id !== Plrs.userid && (b.user.id === message.member.user.id || b.user.id === opponent.id)) {
                                        b.reply({
                                            content: `Wait for your turn... (<@!${Plrs.userid}>)`,
                                            ephemeral: true
                                        });
                                        yield ttt(m);
                                    } else if (b.user.id !== Plrs.userid && (b.user.id !== message.member.user.id && b.user.id !== opponent.id)) {
                                        b.reply({
                                ...
                                tttcollector.on('end', (collected, reason) => {
                                    const map = (obj, func) => Object.entries(obj).reduce((prev, [key, value]) => (Object.assign(Object.assign({}, prev), { [key]: func(key, value) })), {});
                                        const objectFilter = (obj, predicate) => Object.keys(obj)
                                            .filter((key) => predicate(obj[key])) // @ts-ignore
                                            .reduce((res, key) => ((res[key] = obj[key]), res), {});
                                        const Filer = objectFilter(map(Plrs, (_, elem) => elem.emoji == dashmoji), (num) => num == true);
                                    if (collected.size === 0 && reason == 'time' && Object.keys(Filer).length != 0)
                                        m.edit({
                                            content: `<@!${Plrs.userid}> didn\'t react in time! (30s)`,
                                            embeds: [
                                                epm.setDescription(`<@!${Plrs.userid}> didn\'t react in time! (30s)`)
                                            ],
                                            components: disableButtons(buttons)
                                        });
                                });
                ...
                collector.on('end', (collected, reason) => {
                    ...
                    if (reason == 'time') {
                        ...
                        m.edit({
                            ...
                            embeds: [embed],
                            components: disableButtons(m.components)
                        });
                    }
